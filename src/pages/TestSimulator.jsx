...
export default function TestSimulator() {
  ...
  const bulkAddVisitors = async () => {
    ...
    try {
      const promises = [];
      let createdVisitorsCount = 0; // Track actual visitors created
      for (let i = 0; i < bulkCount; i++) {
        const variant = assignVariant();
        if (!variant) {
          console.warn('Skipping visitor creation due to no variant assignment.');
          continue;
        }
        ...
        const visitorData = {
          ...
          ab_test_id: selectedTest,
          assigned_variant_id: variant.id,
          first_seen_date: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
          last_seen_date: new Date().toISOString(),
          device_type: deviceType,
          browser: getRandomElement(browsers),
          referrer_source: getRandomElement(referrerSources),
      ...
      await Promise.all([calculateStats(selectedTest, variants), refreshQuota()]);
      toast.success(`${createdVisitorsCount} visitors added successfully`);
    } catch (error) {
  ...
  const generateRealisticData = async () => {
    ...
    try {
      ...
      for (let i = 0; i < baseVisitorCount; i++) {
        const variant = assignVariant();
        if (!variant) {
          console.warn('Skipping realistic data visitor creation due to no variant assignment.');
          continue;
        }
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <Button
                onClick={addVisitor}
                disabled={!selectedTest || isLoading || isTestCompletedOrArchived}
                className="w-full"
              >
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    max="1000"
                    className="flex-1"
                    disabled={isLoading || isTestCompletedOrArchived}
                  />
                  <Button
                    onClick={bulkAddVisitors}
                    disabled={!selectedTest || isLoading || !bulkCount || isTestCompletedOrArchived}
                    variant="outline"
                  >
                    Add
                  </Button>
                  ...
                  <div className="flex h-8 rounded-lg overflow-hidden border border-slate-200">
                    {variants.map((variant, index) => (
                      <div
                        ...
                        className={`${index === 0 ? 'bg-slate-600' : variantColors[Math.min(index - 1, variantColors.length - 1)]} flex items-center justify-center text-white text-xs font-medium transition-all duration-300`}
                      >
