...
export default function Onboarding() {
  ...
  const mapTemplateToTest = (t) => {
    ...
    return {
      ...
      test_type: (t?.config?.test_type === 'url_split' || t?.config?.test_type === 'code_variant')
        ? t.config.test_type
        : 'code_variant',
      test_url: websiteUrl || '',
  ...
  const downloadSnippet = () => {
    const blob = new Blob([generateSnippet()], { type: "text/plain" });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "quicksig-tracking.js";
  ...
  const generateSnippet = () => {
    ...
    return `<!-- QuickSig Tracking -->
<script>
  (function(){
    window.QUICKSIG = window.QUICKSIG || {};
    QUICKSIG.init = { orgId: "${org}", testId: "${testId}" };
